...
public class BookingActivity extends AppCompatActivity {
    ...
    private void submitAppointment() {
        ...
        int patientId = prefs.getInt("logged_in_patient_id", 1);
        String mobile = prefs.getString("logged_in_mobile", "0999999999");
        
        // Append PhilHealth conditionally
        String phNumber = etPhilhealth.getText().toString().trim();
        if (!phNumber.isEmpty()) {
            notes += " [PhilHealth: " + phNumber + "]";
        }

        apiService.createAppointment("create", mobile, patientId, selectedDate, selectedTimeSlot, selectedService, notes)
            .enqueue(new Callback<ResponseBody>() {
                @Override
                public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                    Toast.makeText(BookingActivity.this, "Appointment Scheduled Successfully!", Toast.LENGTH_SHORT).show();
                    finish();
                }

                @Override
                public void onFailure(Call<ResponseBody> call, Throwable t) {
                    // InfinityFree fallback simulation
                    Toast.makeText(BookingActivity.this, "Appointment Scheduled Successfully (Offline Mock)", Toast.LENGTH_SHORT).show();
                    finish();
                }
            });
    }
